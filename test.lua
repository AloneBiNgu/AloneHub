...
local PandaAuth =
    loadstring(
    game:HttpGet(
        "https://raw.githubusercontent.com/Panda-Repositories/PandaKS_Libraries/main/library/LuaLib/ROBLOX/PandaBetaLib.lua"
    )
)()
KeySys = {}
...
function KeySys.Main(config)
    ...
    text_label.Text = tostring(config.HubName) .. " - Key System"
    ...
    text_label_2.Text =
        "To use the free version of " .. config.HubName .. " you need a key. Click 'Get Key' button to get your key!"
    ...
    check_key.MouseButton1Click:Connect(
        function()
            if PandaAuth:ValidateKey(ServiceID, text_box.Text) then
                if writefile then
                    writefile(config.HubName .. "_key.txt", text_box.Text)
                end
    ...
    get_key.MouseButton1Click:Connect(
        function()
